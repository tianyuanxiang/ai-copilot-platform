...
AGENT_TOOL_SCHEMAS = [
    ...
    _function(
        ...
        "查询传感器/设备测点的原始 TDengine 时序数据。用户需要具体时间点、曲线数据、原始测点值或分页数据时调用；如果用户要趋势摘要、阈值解析或风险判断，优先调用 compare_sensor_trend。",
        {
            ...
            "field": {"type": "array", "items": {"type": "string"}, "maxItems": 8, "description": "测点字段名列表，最多 8 个；未知字段时先查元数据或澄清。"},
    ...
    _function(
        ...
        "对传感器测点做趋势对比、阈值解析和风险判断。用户询问“是否异常、趋势如何、风险等级、相比之前是否升高/下降、是否超过阈值”时调用；不要用于大量原始点位导出。",
        {
            ...
            "field": {"type": "array", "items": {"type": "string"}, "maxItems": 8, "description": "需要分析的测点字段，最多 8 个。"},
